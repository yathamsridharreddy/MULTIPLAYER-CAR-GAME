...
[![Tests](https://img.shields.io/badge/Tests-457%20passed%20%2F%200%20failed-00f59b?style=flat-square&logo=node.js)](test/)
...
- **v143** — the identity, drawn instead of decoration: the logo had been a 1254¹² raster (1.52 MB, no alpha channel, one size only) and the favicon was a `<text>` element containing a car emoji, which is a font glyph — so the tab icon was whatever emoji font the device happened to have and simply vanished where the codepoint was missing. The mark is now an original vector monogram built from coordinates (`tools/logo-forge/build_logo.py`): squared, chamfered letterforms slanted 13°, with every stroke and every gap held to ≥ 2px at 16px so the R counter survives the favicon. It ships as 11 SVG masters (0 raster, 0 metadata, no font embedded), 32 transparent RGBA PNGs across four colourways × eight sizes, seven opaque app icons and three splash marks; `public/img/logo.svg` and `logo-horizontal.svg` are used in the tab, the boot splash and the lobby header, and the maskable PWA icon is scaled to sit inside Android's 80% safe circle. Net effect on the repo: the logo went from 1.52 MB to 15 KB and gained a real alpha channel. Social preview cards still carry the previous artwork
